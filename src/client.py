...
class Camera:
	...
	def initiate(self):
		...
		pipeline_profile = self.pipe.start(config)
	...
	def get_frame(self):
		...
		self.left = np.asanyarray(f1.get_data())
		
		f2 = frames.get_infrared_frame(2)
		self.right = np.asanyarray(f2.get_data())
	...
	def leftimage(self):
		...
		while True:
			try:
				...
				while not self.got_left:
					time.sleep(1e-4)
				self.got_left=False
				
				if message == "read":
					socket.send_pyobj({'img':cv2.imencode('.jpeg', self.left)[1]})
				if message == 'stop':
					socket.send_string('Stopping Server')
					break
					
				while not self.right_sent:
					time.sleep(1e-4)
				...
				print(f"1: An error occurred: {str(e)}")
				self.pipe.stop()
				self.initiate()
	...
	def rightimage(self):
		...
		while True:
			try:
				...
				if message2 == "read2":
					socket2.send_pyobj({'img2':cv2.imencode('.jpeg', self.right)[1]})
				if message2 == 'stop':
					socket2.send_string('Stopping Server')
					break
				self.right_sent = True
				
			except RuntimeError as e:
				print(f"2: An error occurred: {str(e)}")
		
	def temperature(self):
		context = zmq.Context()
		socket3 = context.socket(zmq.REP)
		print('Binding to port 5557')
		...
		while True:
			...
			self.data = self.bus.read_i2c_block_data(0x38,0x00)
			self.temp = ((self.data[3] & 0x0F) << 16) | (self.data[4] << 8) | self.data[5]
			self.ctemp = ((self.temp*200) / 1048576) - 50
			self.ctemp = (u'Temperature: {0:.1f}°C'.format(self.ctemp))
			if message3 == "read3":
				socket3.send_string(self.ctemp)
...
def IMU():
    ...
    try:
        ...
        if 'echo' in actions:
            ...
            try:
                while True:
                    message4 = socket4.recv_string()
                    if message4 == 'read4':
                        socket4.send_string(str(mt.read_measurement(mode, settings)))
...
def get_settings(arg):
    ...
    for c in arg:
        if c == 't':
            ...
            return
    settings = orient_mode | calib_mode
    return settings

def GPS():
    context = zmq.Context()
    socket5 = context.socket(zmq.REP)
    print('Binding to port 5559')
    socket5.bind("tcp://*:5559")
    print("Connected")
    ser = serial.Serial('/dev/ttyUSB1', 115200, timeout=0, parity=serial.PARITY_EVEN, rtscts=1)

    t = ""
    with open('1.txt', 'w') as f1, open('LL.txt', 'w') as f2, open('Deg.txt', 'w') as f3:
        while True:
            cc = ser.read().decode('utf-8')
            t += cc
            if cc == '\n':
                #print(t)  # Print the received data
                f1.write(t)  # Write the received data to 1.txt
                f1.flush()  # Flush the data to the file
                if t.startswith('$GNGLL'):
                    data = t.split(',')
                    if len(data) >= 7:
                        lat_lon = data[1].split() + data[3].split()  # Split latitude and longitude using spaces
                        lat_lon_str = ' '.join(lat_lon) + "\n"
                        #print(lat_lon_str)  # Print the extracted latitude and longitude
                    ...
                    data = t.split(',')
                    if len(data) >= 8:
                        track_made_good = data[8]  # Extract Track made good in degrees True
                        track_made_good_str = track_made_good + "\n"
                        #print(track_made_good_str)  # Print the extracted track made good
                        f3.write(track_made_good_str)  # Write Track made good to Deg.txt
                        f3.flush()  # Flush the data to the file
                t = ""
...
def sonar():
    ...
    print('Binding to port 5560')
    socket6.bind("tcp://*:5560")
    print("Connected")
    ser = serial.Serial('/dev/ttyUSB2', baudrate=115200, parity=serial.PARITY_NONE)

    output_file = open('depth_data.txt', 'w')  # Open the file in 'append' mode

    
    try:
        while True:
            data_line = ser.readline()
            if b'$SDDBT' in data_line:
                decoded_line = '$' + data_line.decode('utf-8', 'ignore').split('$')[-1]
                #print(decoded_line)
                
                # Extract depth in meters (modify this part based on the line structure)
                depth_meters = decoded_line.split(',')[3]
                message6 = socket6.recv_string()
                if message6 == 'read6':
                    socket6.send_string(f"Depth in meters: {depth_meters}")
                # Write depth to the text file and flush the buffer
                output_file.write(f"{depth_meters}\n")
                output_file.flush()
    except KeyboardInterrupt:
